...
function DistributionPanel({ title, values, order }: { title: string; values: Record<string, number>; order?: string[] }) {
  return <section className="panel overview-distribution-panel"><PanelHeading title={title}/><DistributionList values={values} label={`${title} exact counts`} order={order}/></section>;
}
...
function PersistedTimeline({ summary, onSelect }: { summary: DashboardSummary; onSelect: (start: string, bucketMinutes?: number) => void }) {
  ...
  return <figure className="overview-timeline">
    <figcaption><strong>Window pattern</strong><span>{peak?.total ? `Peak ${peak.total.toLocaleString()} at ${new Date(peak.bucket_start).toLocaleString()} · ${criticalTotal.toLocaleString()} critical across the window` : "No alert activity in this window"}</span></figcaption>
    ...
    <SeverityTimelineChart rows={rows} bucketMinutes={summary.scope.bucket_minutes} onSelect={onSelect}/>
